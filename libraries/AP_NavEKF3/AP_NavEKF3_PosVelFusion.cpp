...
void NavEKF3_core::selectHeightForFusion()
{
    ...
    if ((frontend->sources.getPosZSource() == AP_NavEKF_Source::SourceZ::NONE)) {
        ...
        // Allow switch to rangefinder when low if ground was recently valid OR we have fresh rangefinder data (e.g. after second takeoff).
        bool belowLowerSwHgt = ((terrainState - stateStruct.position.z) < 0.7f * rangeMaxUse) && ((imuSampleTime_ms - gndHgtValidTime_ms < 1000) || rangeFinderDataIsFresh);
